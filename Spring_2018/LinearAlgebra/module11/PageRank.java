...
//
// You need to have lintool.jar in your CLASSPATH
...
public class PageRank extends JPanel implements MouseListener {
    ...
    //------------------------------------------------------------------
    // Theoretical computation of eigenvector x using power method.
    ...
    void theory() {
        ...
        // If the theory is correct, both x and visitProbEstimate
        // should be close.
        DuquetteLinTool linTool = new DuquetteLinTool();

        System.out.println("\nv_i(t)/t for t=" + numSteps);
        for (int i = 0; i < numNodes; i++) {
            System.out.printf("i=%3d: v_i(t)/t=%6.4f\n", i, visitProbEstimate[i]);
        }
        ...
        double[][] A = linTool.transpose(P);

        // The power method
        double[] x = pageProbabilitiesViaPowerMethod(A);

        // Now we'll compare the two theoretical values with the
        // actual simulation estimates.
        System.out.println("\nComparison: simulation vs. theory:");
        for (int i = 0; i < numNodes; i++) {
            System.out.printf("i=%3d: v_i(t)/t=%6.4f  Powermethod=%6.4f\n", i, visitProbEstimate[i], x[i]);
        }

    }


    public double[] pageProbabilitiesViaPowerMethod(double[][] A) {
        DuquetteLinTool linTool = new DuquetteLinTool();

        // Now start with some initial prob vector p
        int n = A[0].length;
        ...
        for (int k = 0; k < numIter; k++) {
            x = linTool.matrixVectorMult(A, x);
        }
